...
namespace RandomPetGenerator.Controllers
{
    ...
    public class PetNameGeneratorController : ControllerBase
    {
        ...
        {
            { "dog", new List<string> { "Yuna", "Yoru", "Yumi", "Rock", "Dagger" , "Luna" , "Tiger" , "Doom" , "Phantom" } },
            { "cat", new List<string> { "Whiskers", "Mittens", "Luna", "Simba", "Tiger" , "Puck" , "Morp" , "Qop" , "Zeus" } },
            { "bird", new List<string> { "Tweety", "Sky", "Chirpy", "Raven", "Sunny" , "Tinker" , "Sniper" , "Spec" , "Legion" } }
        };
        ...
        public IActionResult GeneratePetName([FromBody] PetNameRequest request)
        {
            ...
            {
                return BadRequest(new { error = "Invalid animal type , values: dog, cat, bird." });
            }
